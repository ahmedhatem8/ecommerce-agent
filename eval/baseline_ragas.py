"""
eval/baseline_ragas.py — Baseline (naive dense) RAG evaluation
20 questions across 3 difficulty levels. Saves results to eval/baseline_rag_results.json.
...
OUTPUT_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "baseline_rag_results.json")

# ── 20 questions — medium / hard / very_hard ─────────────────────────────────

EVAL_QUESTIONS = [

    # ── Medium (5 questions) — direct, single-document lookups ───────────────
    ...
    # ── Hard (8 questions) — edge cases, policy thresholds, multi-detail ─────
    ...
    # ── Very Hard (7 questions) — multi-document, calculation, hallucination traps ──
...
def run():
    print("=" * 60)
    print("  Baseline RAG Evaluation — 20 Questions")
    print("=" * 60)
    ...
    for i, item in enumerate(EVAL_QUESTIONS, 1):
        ...
        sources = retriever.invoke(item["question"])
    ...
    output = {
        "run_name":  "Baseline RAG (Dense Retrieval Only)",
